		...
		echo $umData;
			...
			if($wise=="date"){
				...
				if($page<0){
					...
					$sql1.= "AND `date_time` < now() ORDER BY `date_time` DESC,`series_name`limit 50 OFFSET 0 ";			
					$sql2.= "AND `date_time` >= now() ORDER BY `date_time`,`series_name`limit 50 OFFSET 0";		
...
//				echo $sql."<hr />";
				...
				$memcached->set($key,json_encode($arr),600);
			}
